...
export class DistributionService {
  ...
  async getDistributionOverview() {
    ...
    // 3. Fetch current assigned rider counts per zone today
    ...
    const fullZonesOverview = zonesOverview.map((z) => {
      ...
      const maxCap = activeZone.max_capacity !== undefined && activeZone.max_capacity !== null
  ...
  async autoDistributeRiders() {
    ...
    console.log("--------------------------------------------------------------------------------");
    console.log(`📊 Total Rider Ditugaskan : ${assignments.length}`);
    console.log(`📊 Total Rider Tercecer   : ${unassignedCount}`);
    console.log(`📌 STATUS KAPASITAS ZONA  : ${isSufficient ? 'CUKUP ✅' : 'TIDAK MENCUKUPI ⚠️'}`);
    console.log("================================================================================\n");
   ...
   */
  async getMyDutyHistory(riderId, limit = 30) {
    if (!riderId) {
