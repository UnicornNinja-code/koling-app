/*
 * systemReadinessService.js
 * Service for Evaluating Holistic System Readiness & Operational Foundation Checklist
...
export class SystemReadinessService {
  /**
   * Evaluates overall system readiness across all operational pillars
   */
  async evaluateSystemReadiness() {
    ...
    const hubCityName = hubCitySetting?.value || "Sidoarjo";
    const hubAddress = hubAddrSetting?.value || "Jl. Pahlawan No. 1, Sidoarjo, Jawa Timur";
    ...
    // 4. Fetch Active DSS Configuration Profile
    const { rows: dssRows } = await pool.query(`
      SELECT id, name, is_active, consistency_ratio
    ...
    // 5. Fetch Active Armada Units Count
    ...
    // 6. Fetch POI & Category Counts
    const { rows: poiCategoryRows } = await pool.query(`
      SELECT COUNT(*)::int AS count FROM poi_categories WHERE is_active = true;
    `);
    const activeCategoryCount = poiCategoryRows[0]?.count || 0;

    const { rows: poiRows } = await pool.query(`
      SELECT COUNT(*)::int AS count FROM pois WHERE status = 'APPROVED';
    `);
  ...
  /**
   * Updates Central Hub configuration in system_settings
   */
  async updateHubConfig(payload, user = {}) {
    ...
    if (operational_radius_km !== undefined && Number.isFinite(Number(operational_radius_km))) {
      await SystemSettingModel.upsert("OPERATIONAL_RADIUS_KM", String(Number(operational_radius_km)), "Radius Jangkauan Operasional (KM)");
    }
