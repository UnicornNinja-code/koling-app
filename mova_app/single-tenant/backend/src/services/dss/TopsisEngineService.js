...
export class TopsisEngineService {
  ...
  async calculateTopsisRecommendations(options = {}) {
    ...
    // Criteria weights determination (Priority: 1. customWeights preview, 2. Active BWM DB Config, 3. Equal fallback)
    let weights = {
    ...
    // --- STEP 1: FETCH RAW CRITERIA EVALUATIONS & CONSTRUCT DECISION MATRIX (X_m x 6) ---
    ...
    for (let i = 0; i < m; i++) {
      ...
      const rawEval = await rawCriteriaEvaluationService.evaluateZoneRawCriteria(zone.id, {
        timeSlot: activeSlot,
        riderLat,
        riderLon,
      });
      rawEvaluations.push(rawEval);
