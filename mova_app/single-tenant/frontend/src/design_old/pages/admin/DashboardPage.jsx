...
export function DashboardPage() {
  ...
  const fetchDashboardDistribution = useCallback(async () => {
    try {
      ...
      setFetchError("Gagal memperbarui data operasional dari server. Menggunakan data cache lokal.");
    } finally {
  ...
  const { isConnected, liveAlerts } = useDashboardRealtime({
    onRiderMoved: (data) => {
    ...
    onRiderCheckedIn: (data) => {
      fetchDashboardDistribution();
      if (data?.rider_id) {
        setActiveRiders((prev) =>
          prev.map((r) => (r.id === data.rider_id ? { ...r, status: "OPERATING" } : r))
        );
      }
    },
    onRiderCheckedOut: () => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
        ...
        {/* Slot 4: Gerobak / Armada Siaga */}
        <MetricCard
          title="Gerobak / Armada Siaga"
      ...
      {/* ------------------------------------------------------------- */}
      {/* BARIS 3: Peta Spasial Real-Time (Kiri) + Peringkat Area Potensial (Kanan) */}
      {/* ------------------------------------------------------------- */}
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-stretch">
        ...
        {/* Right Column: Peringkat Area Potensial with BWM Info Tooltip */}
        <div className="lg:col-span-4 flex flex-col">
          <Card className="h-full flex flex-col">
            <CardHeader
              title="Peringkat Area Potensial"
      ...
      {fetchError && (
        <div className="p-3.5 rounded-xl bg-amber-50 dark:bg-amber-950/40 border border-amber-200 dark:border-amber-800 text-amber-900 dark:text-amber-200 flex items-center justify-between text-xs shadow-xs">
          <div className="flex items-center gap-2">
            <AlertTriangle className="w-4 h-4 text-amber-600 shrink-0" />
            <span>{fetchError}</span>
          </div>
          <Button size="xs" variant="secondary" onClick={() => fetchDashboardDistribution()}>
            Coba Lagi
          </Button>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-stretch">
        {/* Card 1: Kapasitas Shift */}
        <Card className="h-full flex flex-col justify-between">
          <CardHeader
            title="Kapasitas Shift"
        ...
        {/* Card 2: Status Penugasan Rider */}
        <Card className="h-full flex flex-col justify-between">
          <CardHeader
            title="Status Penugasan Rider"
          ...
          {loadingDistribution ? (
            ...
            <CardContent className="p-4 space-y-2.5 flex-1 flex flex-col justify-between">
              ...
              {(() => {
                const totalRiders = ridersSummary?.total_riders ?? activeRiders.length;
                const operating = ridersSummary?.operating ?? activeRiders.filter((r) => r.status === "OPERATING" || r.status === "ON_TIME").length;
                const plotted = ridersSummary?.plotted ?? activeRiders.filter((r) => r.status === "PLOTTED").length;
                const waiting = ridersSummary?.waiting_queue ?? (distributionData?.total_waiting_riders ?? 0);
                const deviated = ridersSummary?.deviated ?? activeRiders.filter((r) => r.status === "DEVIATION").length;
                const unconfirmed = ridersSummary?.unconfirmed ?? activeRiders.filter((r) => r.status === "UNCONFIRMED").length;
              ...
              <div className="space-y-1.5 flex-1 overflow-y-auto max-h-[190px] scrollbar-thin">
                ...
                {/* 2. Active & Plotted Riders with real-time sync */}
                {activeRiders.slice(0, distributionData?.waiting_queue?.length ? 3 : 4).map((r, idx) => {
                  const status = r.status || (idx === 0 ? "ON_TIME" : idx === 1 ? "PLOTTED" : idx === 2 ? "DEVIATION" : "UNCONFIRMED");
                  ...
                  return (
                    ...
                    >
                      <div>
                        ...
                        <div className="text-[11px] text-slate-500 mt-0.5">
                          {r.zoneName || "Zona Sidoarjo"} • <span>{r.armadaCode || "AR-01"}</span>
                        </div>
                      </div>
                      <div className="text-right">
