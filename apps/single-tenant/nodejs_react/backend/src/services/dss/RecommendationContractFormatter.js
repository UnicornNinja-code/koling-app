...
export class RecommendationContractFormatter {
  ...
  static generateReasoning(rawScores = {}, matrixAvg = {}, rank = 1, preferenceScore = 0, zoneName = "Zona") {
    ...
    const scorePct = (preferenceScore * 100).toFixed(2);
    const summary = `${zoneName} berada di Peringkat #${rank} dengan skor preferensi TOPSIS ${preferenceScore.toFixed(4)} (${scorePct}%). Faktor unggulan: ${strongFactors.slice(0, 3).join(", ")}.`;
