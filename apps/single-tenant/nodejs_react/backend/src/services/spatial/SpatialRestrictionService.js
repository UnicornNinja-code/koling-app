/*
 * SpatialRestrictionService.js
 * Single Source of Truth (SSOT) for Spatial PostGIS Validation:
 * 1. Zone Polygon (Self-intersection ST_IsValid, Area Bounds, Active Overlaps, Toll Road Intersections)
 * 2. Candidate Selling Spot (Zone Containment ST_Contains, Protocol Road Prohibitions)
 * 3. Fleet Rider Stop / Rest Area Validation
 */
...
export class SpatialRestrictionService {
  ...
  /**
   * Format polygon into standard GeoJSON string
   */
  ...
  /**
   * 1. Validate Operational Zone Polygon (Dry-Run / Pre-Save)
   */
  async validateZonePolygon(polygon, excludeZoneId = null) {
    ...
    const MIN_AREA_SQM = 1000;      // 0.1 Hektar
    const MAX_AREA_SQM = 5000000;   // 500 Hektar
    ...
    const areaValid = areaSqm >= MIN_AREA_SQM && areaSqm <= MAX_AREA_SQM;
    if (!areaValid) {
      isValid = false;
      warnings.push(`Luas area (${areaSqm.toFixed(1)} m²) berada di luar batas operasional (Min: ${MIN_AREA_SQM} m², Max: ${MAX_AREA_SQM} m²).`);
    }

    const overlappingZones = res.overlapping_zones || [];
    if (overlappingZones.length > 0) {
      isValid = false;
      warnings.push(`Poligon bertumpukan (overlap) dengan ${overlappingZones.length} zona aktif lain: ${overlappingZones.map(z => z.name).join(", ")}`);
    }
  ...
  /**
   * 2. Validate Candidate Selling Spot (Containment in Zone & 10m Buffer from Prohibited Protocol Roads)
   */
  async validateCandidateSpot(latitude, longitude, zoneId) {
    ...
    // Step B: Check Prohibited Protocol Roads (10 meter buffer)
    const roadQuery = `
    ...
    if (roadRows.length > 0) {
      return {
        ...
        rejection_reason: roadRows[0].restriction_type === 'PROHIBITED_TOLL_ROAD' ? "PROHIBITED_TOLL_ROAD" : "PROHIBITED_ROAD",
