/*
 * DSSPerformanceService.js
 * Domain Service for Milestone B-12: Plan-vs-Actual DSS Effectiveness Analytics
 * Evaluates whether mathematical TOPSIS rankings correlate with actual field performance.
 */
...
export class DSSPerformanceService {
  ...
  /**
   * Evaluate Plan vs Actual Performance across TOPSIS Ranks
   */
  async getPlanVsActualAnalysis({ startDate, endDate, date, distributionRunId = null, dssHistoryId = null } = {}) {
    ...
    // Sort by revenue to evaluate rank ordering alignment
    ...
    // Determine qualitative alignment insight
    let topRankAchievedMaxRev = false;
    if (ranksBreakdown.length > 0 && sortedByRevenue.length > 0) {
      topRankAchievedMaxRev = ranksBreakdown[0].zone_id === sortedByRevenue[0].zone_id;
    }
    ...
    return {
      ...
      insights: {
        ...
        top_rank_actual_revenue: topZone ? `Rp ${Math.round(topZone.actual_execution.actual_revenue).toLocaleString("id-ID")}` : "Rp 0",
