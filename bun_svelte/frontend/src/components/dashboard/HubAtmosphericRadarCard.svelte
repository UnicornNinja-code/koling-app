<script lang="ts">
  ...
  const loadData = async () => {
    loading = true;
    try {
      const [weatherRes, configRes] = await Promise.allSettled([
        mapService.getHubWeather('sidoarjo'),
        mapService.getZoneConfig(),
      ]);
      if (weatherRes.status === 'fulfilled') hubWeather = weatherRes.value;
      if (configRes.status === 'fulfilled') zoneConfig = configRes.value;
    } catch (err) {
