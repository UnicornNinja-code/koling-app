<script lang="ts">
  ...
  let zoneWeathers = $state<any[]>([
    { zone_name: 'Zona 01 - Gubeng Grand City', temp: 31, condition: 'Cerah Berawan', rain_prob: 10, wind: 12 },
    { zone_name: 'Zona 02 - Tunjungan Plaza', temp: 32, condition: 'Cerah', rain_prob: 5, wind: 8 },
    { zone_name: 'Zona 03 - Rungkut Industri', temp: 30, condition: 'Berawan Tebal', rain_prob: 25, wind: 15 },
    { zone_name: 'Zona 04 - Wonokromo DTC', temp: 31, condition: 'Cerah', rain_prob: 10, wind: 10 },
  ]);
  ...
  const handleSyncOsm = async () => {
    ...
    successMsg = null;
    try {
      const res = await axiosInstance.post('/pois/sync-osm', { city: 'Surabaya' });
      successMsg = res.data?.message || 'Sinkronisasi POI Overpass OSM Surabaya berhasil diselesaikan!';
      await loadPois();
    } catch (err: any) {
...
<div class="space-y-6 max-w-7xl mx-auto pb-12 font-outfit-400">
    ...
    <div class="flex items-center gap-3">
      ...
      <div>
        ...
        <p class="text-xs text-zinc-400">
          Katalog titik pusat keramaian (POI) dan telemetri cuaca per zona operasional Surabaya
        </p>
      </div>
    </div>
  ...
  <!-- SECTION 1: Cuaca Per Zona Operasional -->
  <div class="space-y-3">
    <div class="flex items-center justify-between">
      <h3 class="text-sm font-bold text-white flex items-center gap-2">
        <CloudRain class="w-4 h-4 text-sky-400" />
        Telemetri Cuaca Spasial Real-time (Siklus 30 Menit)
      </h3>
      <span class="text-xs text-zinc-400 font-mono">Surabaya Region</span>
    </div>

    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
      {#each zoneWeathers as zw}
        <div class="p-4 rounded-3xl bg-[#131317] border border-[#24242E] space-y-2.5 shadow-lg">
          <div class="flex items-center justify-between">
            <span class="text-xs font-bold text-white truncate">{zw.zone_name}</span>
            <Sun class="w-4 h-4 text-amber-400" />
          </div>

          <div class="flex items-baseline gap-2">
            <span class="text-2xl font-bold text-white font-mono">{zw.temp}°C</span>
            <span class="text-xs text-zinc-400">{zw.condition}</span>
          </div>
          ...
          <div class="grid grid-cols-2 gap-2 pt-2 border-t border-[#22222A] text-[11px] text-zinc-400">
            <div class="flex items-center gap-1">
              ...
              <span>Hujan: {zw.rain_prob}%</span>
            </div>
            <div class="flex items-center gap-1">
              <Wind class="w-3.5 h-3.5 text-teal-400" />
              <span>Angin: {zw.wind}km/h</span>
            </div>
          </div>
        </div>
      {/each}
        ...
        <tbody class="divide-y divide-[#202028]">
          {#each filteredPois as poi}
            <tr class="hover:bg-[#181822] transition-colors">
              ...
              <td class="p-3 text-zinc-300">{poi.zone_name || 'Zona 01 - Gubeng'}</td>
